...
static BLUEPRINTS: LazyLock<Vec<(BuildingKind, Blueprint)>> = LazyLock::new(|| {
    ...
    vec![
        ...
        // The test lot: three tiles of ring in front of a pavilion, open
        // all day for an outing, so a town's cars have somewhere to be
        // watched parking. Its slots are its spots, like any lot's.
        (Lot, Blueprint {
            class: Commerce, homes: 0, jobs: 0, size: (3, 1), lot: (3, 1), weight: 0.0, tilt: &[], by_hand: true,
            stock: 0, answers: None, vehicles: 0,
            taps: vec![outing(Leisure, always(), 12)],
        }),
    ]
});
