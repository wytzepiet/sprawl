        ...
        <For each={modes}>
          {(m) => {
            ...
            return (
              ...
              >
                <Icon size={18} stroke-width={buildMode() === mode.id ? 2.25 : 1.5} />
                <span class={`text-xs font-semibold tracking-wide uppercase transition-all duration-300
                  ${buildMode() === mode.id ? "opacity-100 max-w-20" : "opacity-0 max-w-0 overflow-hidden group-hover:opacity-60 group-hover:max-w-20"}`}>
                  {mode.label}
                </span>
                <kbd class={`pointer-events-none absolute -top-1.5 -right-0.5 text-[9px] font-mono px-1 py-0.5 rounded-md bg-white border border-black/[0.06] text-stone-400 leading-none shadow-sm transition-opacity
                  ${buildMode() === mode.id ? "opacity-0" : "opacity-0 group-hover:opacity-80"}`}>
                  {mode.key}
                </kbd>
              </button>
            );
